    ...
    const handleDelete = async (bookId) => {
        try {
            const confirm = await Swal.fire({
                title: "متأكد؟",
                text: "مش هتقدر ترجع في الخطوة دي!",
                icon: "warning",
            ...
            if (confirm.isConfirmed) {
                ...
                Swal.fire("اتمسح!", "الكتاب اتشال من القائمة بنجاح.", "success");
            }
        } catch (error) {
            Swal.fire("خطأ", "فشلنا في مسح الكتاب", "error");
        }
    };
                ...
                <aside className="w-64 flex-shrink-0 hidden lg:block">
                    <div className="sticky top-32 space-y-10">
                        <div className="space-y-4">
                            <h1 className="font-notoSerif text-2xl font-bold text-primary">ستوديو البائع</h1>
                        ...
                        <nav className="space-y-4">
                            <Link className="flex items-center gap-3 p-3 bg-surface-container-highest text-primary rounded-xl font-bold transition-all duration-200" to="/seller-dashboard">
                                ...
                                <span className="text-sm">إدارة المجموعة</span>
                            </Link>
                            <Link className="flex items-center gap-3 p-3 text-on-surface-variant hover:bg-surface-container-high transition-all duration-200 rounded-xl" to="/add-book">
                                ...
                                <span className="text-sm">عرض كتاب جديد</span>
                            </Link>
                            <Link className="flex items-center gap-3 p-3 text-on-surface-variant hover:bg-surface-container-high transition-all duration-200 rounded-xl" to="/orders">
                                ...
                                <span className="text-sm">نظرة على المبيعات</span>
                            </Link>
                        </nav>
                        <div className="p-6 bg-primary rounded-2xl text-on-primary">
                            <h4 className="font-bold mb-2">نصيحة للبائع</h4>
                            <p className="text-xs opacity-80 leading-relaxed">الصور الكويسة لغلاف الكتاب وكعبه بتزود فرصة البيع بنسبة 40%.</p>
                        </div>
                    </div>
                </aside>
                ...
                <div className="flex-grow">
                    <header className="flex justify-between items-end mb-12">
                        <div className="text-right">
                            <span className="text-tertiary-container font-bold text-xs tracking-widest uppercase mb-2 block">كاتالوج كتبك</span>
                            <h2 className="font-notoSerif text-4xl text-primary italic leading-tight">بندير مجموعتك الخاصة.</h2>
                        </div>
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-3 gap-8 mb-12">
                        <div className="bg-surface-container-low p-8 rounded-2xl relative overflow-hidden text-right">
                            <span className="material-symbols-outlined absolute -left-4 -top-4 text-primary/5 !text-9xl">auto_stories</span>
                            <p className="text-xs uppercase tracking-widest text-on-surface-variant mb-2">كتب معروضة</p>
                            <p className="font-notoSerif text-3xl text-primary">{activeListingsCount}</p>
                        </div>
                        <div className="bg-surface-container-low p-8 rounded-2xl relative overflow-hidden text-right">
                            <span className="material-symbols-outlined absolute -left-4 -top-4 text-primary/5 !text-9xl">sell</span>
                            <p className="text-xs uppercase tracking-widest text-on-surface-variant mb-2">كتب اتباعت</p>
                            <p className="font-notoSerif text-3xl text-primary">{soldBooksCount}</p>
                        </div>
                        <div className="bg-surface-container-low p-8 rounded-2xl relative overflow-hidden text-right">
                            <span className="material-symbols-outlined absolute -left-4 -top-4 text-primary/5 !text-9xl">payments</span>
                            <p className="text-xs uppercase tracking-widest text-on-surface-variant mb-2">إجمالي الأرباح</p>
                            <p className="font-notoSerif text-3xl text-primary">{totalEarnings} ج.م</p>
                        </div>
                    </div>

                    {/* Book List */}
                    <div className="bg-surface-container-lowest p-8 rounded-2xl shadow-sm border border-outline-variant/10 overflow-hidden text-right">
                        <h3 className="font-notoSerif text-2xl text-primary italic mb-8">كتبي المعروضة</h3>
                        ...
                        {loading ? (
                                ...
                                <table className="w-full text-right">
                                    <thead>
                                        <tr className="border-b border-outline-variant/20">
                                            <th className="pb-4 text-xs font-bold uppercase tracking-widest text-outline">تفاصيل الكتاب</th>
                                            <th className="pb-4 text-xs font-bold uppercase tracking-widest text-outline">السعر</th>
                                            <th className="pb-4 text-xs font-bold uppercase tracking-widest text-outline">الحالة</th>
                            ...
                            <div className="text-center py-12">
                                <p className="text-on-surface-variant">لسه ما عرضتش أي كتب للبيع.</p>
                                <Link to="/add-book" className="text-primary font-bold hover:underline mt-2 inline-block">اعرض أول كتاب ليك دلوقت</Link>
                            </div>
                        )}
                    </div>
                </div>
